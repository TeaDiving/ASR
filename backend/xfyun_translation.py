...
def read_xfyun_credentials() -> XFYUNCredentials:
    ...
    api_key = (
        os.getenv("XF_TRANSLATE_API_KEY")
        or os.getenv("XF_APIKEY")
        or os.getenv("XFYUN_API_KEY")
    )
    api_secret = (
        os.getenv("XF_TRANSLATE_API_SECRET")
        or os.getenv("XF_SECRET")
        or os.getenv("XFYUN_API_SECRET")
    )
...
def build_translation_payload(text: str, app_id: str, from_lang: str = "en") -> dict[str, Any]:
    return {
        "common": {
            "app_id": app_id,
        },
...
def parse_translation_response(response_data: dict[str, Any]) -> str:
    if response_data.get("code") != 0:
        raise TranslationError(f"XFYUN translation returned error code {response_data.get('code')}: {response_data.get('message')}")
...
async def translate_text(
    ...
    # Use standard Whisper codes directly. If already Chinese, just return text.
    if from_lang in ["zh", "cn"]:
        return text

    if credentials is None:
        credentials = read_xfyun_credentials()

    # The user was right: iFlytek ITS v2 uses standard codes like 'ja' for Japanese.
    # No mapping needed for most languages.
    target_from_lang = from_lang

    payload = build_translation_payload(text, credentials.app_id, from_lang=target_from_lang)
